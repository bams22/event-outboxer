...
public record WorkerId(String value) {
    ...
    public WorkerId {
        ...
        if (value.length() > MAX_LENGTH) {
            throw new IllegalArgumentException(
                    "WorkerId value must be at most "
                            + MAX_LENGTH
                            + " characters, got "
                            + value.length());
        }
    ...
    /**
     * Generates a {@code WorkerId} in the default {@code {hostname}-{pid}-{uuid8}} form. The
     * hostname is truncated if necessary to keep the overall length within {@link #MAX_LENGTH}.
     */
