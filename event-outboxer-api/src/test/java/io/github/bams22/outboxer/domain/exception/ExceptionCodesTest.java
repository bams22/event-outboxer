...
class ExceptionCodesTest {
  ...
  void publishValidationCarriesCode() {
    ...
    assertThat(e).hasMessageStartingWith("OUTBOX-101:").hasMessageContaining("missing eventType");
  }

  @Test
  void publishSerializationCarriesCauseAndCode() {
    Throwable cause = new RuntimeException("boom");
    PublishSerializationException e = new PublishSerializationException("serialize failed", cause);
  ...
  void categoryHierarchyIsTraversable() {
    ...
    assertThat(publish).isInstanceOf(PublishException.class).isInstanceOf(OutboxException.class);

    OutboxException storage = new EventStoreException("y", new RuntimeException());
    assertThat(storage).isInstanceOf(StorageException.class).isInstanceOf(OutboxException.class);
  }
}
