...
public final class OutboxEngine {
    ...
    public synchronized void start() {
        ...
        try {
            ...
            state = State.RUNNING;
            log.info(
                    "outbox engine started: worker={} pollers={}", workerInfo.id(), pollers.size());
    ...
    public synchronized void stop(Duration timeout) {
        ...
        try {
            ...
            log.warn(
                    "releaseClaimed failed for {} — leftover PROCESSING rows will be reclaimed by"
                            + " peers via the graceful_stop flag: {}",
                    workerInfo.id(),
                    ex.toString());
        }
        ...
        if (claimsReleased) {
            ...
            log.info(
                    "skipping deregister of {} — worker row left flagged graceful_stop for peer"
                            + " recovery",
                    workerInfo.id());
        }
     ...
     * Throwable)} has fired, regardless of the internal lifecycle phase — this is the signal
     * exposed to the Actuator health indicator and the {@code event_outboxer.engine.state} gauge.
     * Use {@link #isLifecycleActive()} for the "has {@code start()} been called and {@code stop()}
     * not yet completed" question.
     */
    public State state() {
     ...
     * completed — ignoring any intervening crash. Used by Spring Boot's {@code
     * SmartLifecycle.isRunning()} so a crashed engine still goes through the normal shutdown path
     * (deregister worker, drain handlers).
     */
