...
class SerializerResolutionTest {
  ...
  void overrideBeanNameWinsAmongMultipleBeans() {
    ...
    runner
        .withUserConfiguration(NamedOverride.class, ExtraSerializer.class)
        .run(
            ctx -> {
              assertThat(ctx).hasNotFailed();
              OutboxSerializers serializers = ctx.getBean(OutboxSerializers.class);
  ...
  void extraSerializersKeepTheAutoConfiguredWriter() {
    ...
    runner
        .withUserConfiguration(ExtraSerializer.class, SecondExtraSerializer.class)
        .run(
            ctx -> {
              assertThat(ctx).hasNotFailed();
              OutboxSerializers serializers = ctx.getBean(OutboxSerializers.class);
  ...
  void ambiguousRosterFailsFast() {
    ...
    new ApplicationContextRunner()
        ...
        .run(
            ctx -> {
              assertThat(ctx).hasFailed();
              assertThat(ctx.getStartupFailure()).rootCause().hasMessageContaining("write-format");
            });
  }
