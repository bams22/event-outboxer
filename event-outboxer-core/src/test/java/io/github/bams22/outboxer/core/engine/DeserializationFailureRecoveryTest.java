...
class DeserializationFailureRecoveryTest {
                    ...
                    public <T> T deserialize(SerializedPayload payload, Class<T> type) {
                        if (deserializeCalls.incrementAndGet() <= 2) {
                            throw new PayloadDeserializationException(
                                    "unknown property (simulated)", null);
                        }
                ...
                engineWith(flaky)
                        ...
                        .defaultFailureHandler(
                                FailureHandlers.builder()
                                        .withMaxAttempts(
                                                10,
                                                MaxRetriesFailureHandler.ExhaustedAction.DISABLE)
    ...
    @Test
    @DisplayName(
            "poison payload → DISABLED only after the attempt budget, not on the first failure")
        ...
        EventSerializer alwaysBroken =
                new EventSerializer() {
                    ...
                    public <T> T deserialize(SerializedPayload payload, Class<T> type) {
                        ...
                        throw new PayloadDeserializationException(
                                "corrupt payload (simulated)", null);
                    }
                };
        engine =
    ...
    @Test
    @DisplayName(
            "transient store failure while finalizing the deserialization branch → row not"
                    + " stranded")
