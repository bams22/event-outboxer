...
`publishOnly(true)` lifts the handler requirement (ADR-0029): the engine
registers its worker and runs maintenance but starts no pollers, and
ignores any handlers it was given.
...
new OutboxEngineBuilder()
    ...
    .defaultFailureHandler(FailureHandlers.<Object>builder()
        ...
        .withExponentialBackoff(Duration.ofSeconds(30), 2.0, Duration.ofHours(2), 0.2))
                                                // the terminator returns the chain — no build()
