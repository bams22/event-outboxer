...
public final class HandlerDispatcher {
    ...
    private void dispatchRegistered(ClaimedEvent claimed, DispatchHandle handle) {
        ...
        try {
            ...
            // failure is often transient (mixed-version replicas during a rolling deploy reading
            // each
            // other's payload format, or an unknown payload format only newer replicas know) and
            // heals
        ...
        try {
            ...
            try {
                ...
                // A finalize call (markProcessed / markForRetry / markDisabled) failed — most
                // likely a
                // transient storage error. Without recovery the row would stay PROCESSING forever:
                // it is no longer in the in-flight registry (watchdog can't see it) and this worker
                // is
                // alive (orphan recovery won't touch it). Best-effort: release it back to PENDING
                // so
     ...
     * saturated). The event was already claimed, so it must be returned to {@code PENDING} —
    ...
    private void handleUnknownType(ClaimedEvent claimed) {
        ...
        switch (dispatcherConfig.unknownHandlerPolicy()) {
            case SKIP -> {
                ...
                // —
                // otherwise an event published before its handler is deployed gets DISABLED for
                // free.
                boolean ok =
                        store.release(
                                claimed.id(),
            ...
            case FAIL -> {
                ...
                // while
                // this worker lives, neither the watchdog (event never entered the in-flight
                // registry)
    ...
    private Object deserialise(EventHandler<?> handler, ClaimedEvent claimed) {
        ...
        return serializers
                .require(claimed.payloadFormat())
                .deserialize(claimed.payload(), payloadType);
    }
    ...
    @SuppressWarnings({"unchecked", "rawtypes"})
    private EventOutcome invokeHandlerTyped(
            EventHandler handler, EventContext ctx, Object payload) {
    ...
    private void finaliseSuccess(ClaimedEvent claimed) {
        ...
        if (ok) {
            ...
            log.debug(
                    "markProcessed lost race for eventId={} (version bumped by watchdog / orphan"
                            + " recovery)",
                    claimed.id());
        }
    }
        ...
        if (ok) {
            listener.onEventDeleted(
                    new EventDeletedInfo(
                            claimed.id(), claimed.eventType(), claimed.attempts() + 1, reason));
        }
    }
