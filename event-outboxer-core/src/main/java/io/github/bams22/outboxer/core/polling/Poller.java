...
public final class Poller {
     ...
     * wait. Called (via {@link PollerWakeHub}) after a local transaction that published an event of
     * this type commits. Safe from any thread; a wake on a stopped poller is a no-op.
     *
     ...
     * crashes (uncaught {@code Error} from the strategy, JVM thread kill, etc.). Returns {@code
     * false} before {@link #start()} and after {@link #stop()} — only a live-should-be-alive thread
     * that isn't counts as crashed.
     */
            ...
            while (running) {
                try {
                    ...
                    if (claimed == requested && claimed > 0) {
                        ...
                        // immediately. Bounded by free capacity, so this cannot outrun the
                        // handlers.
                        continue;
                    }
    ...
    private boolean submit(ClaimedEvent claimed) {
        try {
            ...
            log.debug(
                    ...
                    claimed.eventType(),
                    ex.toString());
            // The row is already PROCESSING/claimed but never reached the executor — release it
            // back
            // to PENDING, otherwise it stays invisible to watchdog and orphan recovery for as long
            // as
            // this worker lives.
