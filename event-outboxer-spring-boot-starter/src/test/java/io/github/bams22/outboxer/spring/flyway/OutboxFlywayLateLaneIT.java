 ...
 * event-outboxer-lock-postgres-lease} — core and archive migrations up to V010 applied — and adds
...
class OutboxFlywayLateLaneIT {
    ...
    @Test
    @DisplayName("lock lane added after V010 → V005 applies out of order on the next start")
    void lockLaneAdoptedLate() {
        ...
        // 2. The lease module is added later: V005 is below the schema's V010.
        runner.run(
                ctx -> {
                    assertThat(ctx).hasNotFailed();
                    assertThat(versions())
        ...
        runner.run(ctx -> assertThat(versions()).hasSize(10));
    }
