 ...
 * PENDING}, and removes the dead worker rows. The PostgreSQL adapter runs
 * {@code reclaimOrphans} + {@code removeDead} inside a single transaction (see STORAGE.md §Key
 * queries / orphan recovery); for other adapters the two calls are sequential and the second may
 * fail independently.
 */
