...
|---|---|---|
| [2026-09-07, claim-time coalescing as shipped](2026-09-07-laptop-dedup-final.md) | same laptop | the ADR-0037 implementation holds the spike's numbers in every cell (publish p95 6.2 ms with keys, 1.1–1.7 statements per keyed publish, 86 / 28 / 20.5 % coalesced at 8 / 64 / 1 024 keys, keyless path unchanged) and keeps the gains over the insert-time design (hot-key e2e p99 1.97 s → 129 ms, publishers no longer serialized, 3× the coalescing on scattered keys) at the same price (two row writes and ~1 KB of WAL per dense duplicate, ~9× more handler runs on a hot key); an apparent regression of the claim statement's server time was the host — an interleaved A/B of two `RETURNING` shapes shows them equal within a 2× run-to-run noise |
| [2026-09-05, dedup coalescing](2026-09-05-laptop-dedup-claim-time.md) | same laptop | insert-time pin (ADR-0021) versus a claim-time sweep, behind a spike flag: the claim statement is unchanged without duplicates (0.77 ms both) and costs ~5 µs per swept duplicate (2.0 ms at 48 per claim, 5.1 ms at the 1 000-row cap); the pin is the expensive half of the shipped design — a second statement per coalesced publish, publishers of one key serialized on each other's commits (publish p95 12 ms vs 6 ms, +49 % publish rate at 64 keys) and a starved representative under a hot-key burst (e2e p99 1.97 s vs 116 ms); the sweep coalesces twice as much where duplicates are scattered and pays two row writes and ~1 KB of WAL per duplicate where they are dense (2.16 vs 0.05 writes/publish at 8 keys); all cells pass the new per-key freshness invariant; ADR proposed |
| [2026-09-04, bounded lock wait](2026-09-04-laptop-lock-wait.md) | same laptop, plus Redis 7 | the ADR-0035 validation matrix that fixed `lock-wait` at 100 ms: on 5 ms holds the wait removes every busy round trip (lease 6.65 → 5.04 writes/event, Redis 5.38 → 3.01) and raises drain rate by a third; invisible without a key; the feared mixed-workload regression turned out the other way (a 200 ms hot key drains 2.3× faster, the cool keys pay 0.6 s of median); after a crash the wait falls back to the release path; the one loser is an uncapped virtual executor on a hot key, where polling waiters cost more than they save; addendum of 2026-09-05: the Redis pub/sub wake-up adds a fifth of drain rate and halves the tail on the platform executor (464 → 555/s, max 7.3 s → 2.9 s) and lifts the virtual cell 142 → 405/s; the lease locker's LISTEN/NOTIFY variant first halved every cell (a notify inside the delete serializes every release commit of the fleet on `PreCommit_Notify`), then, as an asynchronous statement, measured no gain over polling (274 vs 275/s) and was removed; the Redisson locker (ADR-0036) lands where the Lettuce pub/sub locker lands (513 vs 574/s, same 3.00 writes) at 2.4× the Redis commands per event, `fair` tightening the tail for 12 more |
