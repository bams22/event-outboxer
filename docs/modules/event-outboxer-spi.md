...
| `OutboxAdmin` (+ `AdminCursor`) | postgres, inmemory | operational surface: list / re-enable / purge, keyset-paginated ([ADR-0019](../adr/0019-admin-and-retention-surface.md)) — deliberately separate from `EventStore` so hot-path adapters need not implement it |
