 ...
 * The fail-fast contract (ADR Q3) guarantees that either every event in the batch is saved or
 * none of them is: on failure the caller's transaction is expected to roll back and no events
 * from the batch remain in the outbox.
 */
