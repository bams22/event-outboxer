...
public sealed interface EventOutcome
  ...
  /**
   * Processing completed successfully. The event is removed from the active outbox (or moved to the
   * archive if archiving is enabled).
   */
  record Success() implements EventOutcome {

    /**
     * Canonical zero-state instance. Prefer {@code EventOutcome.Success.INSTANCE} in handler code
  ...
  /**
   * Non-retryable failure — move the event straight to {@code DISABLED}. Use when the handler knows
   * that additional attempts cannot succeed (for example, a permanent {@code 404} from an external
   * API).
   *
   * @param reason human-readable reason written to {@code last_fail_reason}
