 ...
 * Adapters call {@link #get()} before running the relatively expensive aggregate query and
 * {@link #put(OutboxMetricsSnapshot)} to populate it on a miss.
 *
 * <h2>Why it is a port</h2>
 *
 * Health-indicator scrapes across a fleet of pods read independent local caches by default, so
 * each pod reports a snapshot taken within the last TTL — values routinely disagree across
 * replicas which makes operator dashboards look like flapping. A shared cache (Redis, Caffeine
 * with a distributed invalidation bus, Hazelcast, …) collapses all pods onto one snapshot
 * per TTL window and is the motivation for making this pluggable.
 *
 * <h2>Contract</h2>
 *
 * <ul>
 *   <li>{@link #get()} returns {@link Optional#empty()} on a miss, expired entry, or any
 *       deserialisation / backend error (fail-safe: adapters must then fall back to computing
 ...
 *
 * <ul>
 *   <li>{@link #noop()} — disables caching entirely. Every {@link #get()} misses and every
 *       {@link #put(OutboxMetricsSnapshot)} is dropped; adapters compute on each call.
 *   <li>{@link #inMemory(Clock, Duration)} — per-JVM {@code AtomicReference} with TTL. Matches
 *       the pre-SPI behaviour of {@code PostgresEventStore}; suitable for single-pod deployments
 *       or when cross-pod snapshot consistency does not matter.
 * </ul>
 *
...
public interface MetricsSnapshotCache {
   ...
   * decisions so tests can feed a {@code SettableClock}; production code passes
   * {@link Clock#system()}.
   */
