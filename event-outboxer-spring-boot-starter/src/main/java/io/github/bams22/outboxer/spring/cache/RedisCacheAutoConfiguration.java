...
/**
 * Registers {@link LettuceMetricsSnapshotCache} when {@code outbox.cache.type=redis}, the
 * {@code event-outboxer-cache-redis} module is on the classpath, and a
 * {@link StatefulRedisConnection} bean is available (users create one via Lettuce's
 * {@code RedisClient}).
 *
 * <p>TTL comes from {@code outbox.storage.metrics-cache-ttl}; the key prefix from
 * {@code outbox.cache.redis.key-prefix} (default {@code outbox:metrics:}).
 */
@AutoConfiguration
