...
class LockAutoConfigurationSelectionTest {
    ...
    runner
        ...
        .run(
            ctx -> {
              ...
              assertThat(ctx.getBean(EntityLocker.class)).isInstanceOf(PgLeaseEntityLocker.class);
  ...
  void leaseRelaxedSpellings() {
    for (String spelling : new String[] {"postgres_lease", "POSTGRES-LEASE"}) {
      runner
          .withPropertyValues("event-outboxer.lock.type=" + spelling)
          .run(
              ctx ->
                  assertThat(ctx.getBean(EntityLocker.class))
                      .as("spelling '%s' must select the lease locker", spelling)
    ...
    for (String spelling : new String[] {"postgres_advisory", "POSTGRES-ADVISORY"}) {
      runner
          ...
          .run(
              ctx ->
                  assertThat(ctx.getBean(EntityLocker.class))
                      .as("spelling '%s' must select the advisory locker", spelling)
    ...
    InvocationHandler handler =
        (proxy, method, args) -> {
          if (failOnPrepare && method.getName().equals("prepareStatement")) {
            throw new SQLException("relation \"event_outboxer.entity_locks\" does not exist");
          }
          return defaultValue(method, failOnPrepare);
        };
    return Proxy.newProxyInstance(
        LockAutoConfigurationSelectionTest.class.getClassLoader(), new Class<?>[] {type}, handler);
  }
