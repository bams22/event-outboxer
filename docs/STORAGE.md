...
CREATE TABLE event_outboxer.events (
    ...
    payload          JSONB,                 -- text lane (JSON formats); NULL for binary payloads
    payload_binary   BYTEA,                 -- binary lane (ADR-0025); NULL for text payloads
    payload_format   VARCHAR(64)  NOT NULL, -- EventSerializer.format() that wrote the row
    ...
    CONSTRAINT events_payload_exactly_one
        CHECK ((payload IS NULL) <> (payload_binary IS NULL))
);
```

The table is created by `V001` and evolved additively; the dual payload
lane and `payload_format` arrive with `V006__outbox_payload_format.sql`
(ADR-0025).
...
| `payload JSONB` | Text lane: JSON payloads stay readable in psql; GIN indexing over payload fields is possible for debugging. NULL when the payload is binary |
| `payload_binary BYTEA` | Binary lane (ADR-0025): non-JSON serializer output stored verbatim, byte-exact, no Base64 bloat. NULL when the payload is textual |
| `payload_format VARCHAR(64)` | Stable `EventSerializer.format()` id recorded at publish time; the dispatcher selects the deserializer by this value, which keeps rolling deploys and format migrations safe |
| `payload_class VARCHAR(512)` | Publish-time Java class FQN, recorded for diagnostics/auditing only. Never used to pick the deserialization target — that is always `EventHandler.payloadType()` (ADR-0003), so renaming or moving a DTO does not break stored events |
...
### CHECK `events_payload_exactly_one`

The Java-side invariant of `SerializedPayload` (exactly one of text/bytes)
mirrored into the schema: a row carries its payload in exactly one lane.
Both-NULL and both-set inserts are rejected.
...
CREATE TABLE event_outboxer.event_archive (
    id               UUID         PRIMARY KEY,
    event_type       VARCHAR(128) NOT NULL,
    payload          JSONB,                 -- text lane; NULL for binary payloads
    payload_binary   BYTEA,                 -- binary lane (ADR-0025, V007)
